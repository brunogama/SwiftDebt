...
resource: Tests/SwiftDebtKitTests/DebtImprovementWorkflowTests.swift
...
Executable-level coverage runs the built `swift-debt` command with `validate-improvement` and a failing threshold. It asserts exit status `1`, exact failing validation JSON on standard output, and empty standard error.

# Fixtures

The fixture set under `Tests/SwiftDebtKitTests/Fixtures/DebtImprovement/` includes before and after debt reports plus golden comparison and validation outputs. The fixture reports exercise added, removed, changed, unchanged, moved, aggregation, unavailable-evidence, and total-score improvement behavior.

# Citations

[1] [DebtImprovementWorkflowTests.swift](../../Tests/SwiftDebtKitTests/DebtImprovementWorkflowTests.swift)
[2] [Debt improvement fixtures](../../Tests/SwiftDebtKitTests/Fixtures/DebtImprovement/)
