...
resource: Tests/SCMAKitTests/PluginWorkflowTests.swift
...
`PluginWorkflowTests` creates a temporary package that depends on SwiftSCMA by local path and attaches `SCMABuildPlugin` to a demo target. The test runs `swift package scma debt analyze --target Demo` through the command plugin and `swift build` through the build plugin.
...
The build plugin path still asserts a single generated `SCMA.analysis.swift` stamp after a successful build. With `debtValidation.maxScore` set to `0`, the same plugin consumer is expected to fail `swift build` with an `SCMA [DEBT]` diagnostic.

# Citations

[1] [PluginWorkflowTests.swift](../../Tests/SCMAKitTests/PluginWorkflowTests.swift)
