...
### ❌ Use Dev-Based Single PRs When (Default):
- Features are independent
- Simple bug fixes or enhancements
- Multiple agents working in parallel
- User doesn't specify preference

**DEFAULT: branch from and target `dev` (this repository's Git Flow: only release and hotfix stacks target `main`). Prefer single dev-based PRs unless user explicitly requests stacking.**
...
**CRITICAL: Base on previous feature branch, NOT dev**
...
# Base: feature/001-base-auth  ← NOT dev!
...
# Base: feature/002-user-profile  ← NOT dev!
...
1. Merge PR-001 to dev
2. Change PR-002's base to dev (GitHub: "Edit" button on PR)
3. Merge PR-002 to dev
4. Change PR-003's base to dev
5. Merge PR-003 to dev

**Option B: Keep Stack Until End**
1. Merge PR-001 to dev
2. Keep PR-002 based on feature/001 until PR-001 fully merged
3. Then rebase PR-002 onto dev
4. Repeat for PR-003

## Common Pitfalls

### ❌ WRONG: All PRs from dev
```bash
git checkout dev
git checkout -b feature/001-base
# PR: feature/001-base → dev

git checkout dev  # ← WRONG
git checkout -b feature/002-next
# PR: feature/002-next → dev  # ← WRONG (independent, not stacked)
```
...
Stack Sequence:
1. PR-001: feature/001-base-auth → dev (base layer)
...
CRITICAL: PR-002 bases on feature/001-base-auth, NOT on dev
CRITICAL: PR-003 bases on feature/002-user-profile, NOT on dev
```
