...
@main
struct SwiftDebtCommandPlugin: CommandPlugin {
    func performCommand(context: PluginContext, arguments: [String]) async throws {
        ...
        let process = Process()
        process.executableURL = try context.tool(named: "swift-debt").url
        process.arguments = swiftDebtArguments(manifest: manifest.path, remaining: extractor.remainingArguments)
        process.currentDirectoryURL = context.package.directoryURL
        try process.run()
        process.waitUntilExit()
        guard process.terminationReason == .exit && process.terminationStatus == 0 else {
            throw PluginFailure("SwiftDebt failed with exit status \(process.terminationStatus)")
        }
    }

    private func swiftDebtArguments(manifest: String, remaining: [String]) -> [String] {
