...
struct DebtScoringTests {
    @Test func swiftDebtCoreSourcesRemainImportFree() throws {
        let importDeclarations = try swiftDebtCoreSourceFiles().flatMap { file in
            try sourceFindings(in: file) { line, _ in
                line.trimmingCharacters(in: .whitespaces).hasPrefix("import ")
            }
        }

        #expect(importDeclarations.isEmpty, "SwiftDebtCore must not import modules: \(importDeclarations)")
    }
    ...
    @Test func debtScoringSourceDeclaresNoAmbientClockOrMutableState() throws {
        ...
        let scoringSources = try swiftDebtCoreSourceFiles().filter { scoringSourceNames.contains($0.lastPathComponent) }
