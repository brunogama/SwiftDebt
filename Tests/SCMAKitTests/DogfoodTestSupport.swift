...
func runDogfoodSCMA(_ arguments: [String]) throws -> DogfoodCLIRunResult {
    ...
    let process = Process()
    process.executableURL = try dogfoodSCMAExecutableURL()
...
func makeDogfoodTemporaryDirectory(named name: String) throws -> URL {
    let directory = FileManager.default.temporaryDirectory
        .appendingPathComponent("swiftscma-dogfood-\(name)-\(UUID().uuidString)", isDirectory: true)
    try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
    return directory
}

private func dogfoodSCMAExecutableURL() throws -> URL {
    let direct = dogfoodRepositoryRoot.appendingPathComponent(".build/debug/scma")
    ...
    guard let executable = matches.first else {
        throw DogfoodTestError("Could not locate built scma executable")
    }
    return executable
}
