...
struct HistoryReferenceCLIWorkflowTests {
    ...
    @Test func configuredReferenceTimeControlsGitRecency() throws {
        ...
        """.write(
            to: directory.appendingPathComponent(".scma.json"),
            atomically: true,
            encoding: .utf8
        )
    ...
    @Test func invalidCLIReferenceTimeExitsTwoWithActionableError() throws {
        let result = try runSCMA(["debt", "analyze", "--debt-reference-time", "not-a-timestamp"])
    ...
    @Test func invalidConfiguredReferenceTimeExitsTwoAndNamesConfiguration() throws {
        ...
        let result = try runSCMA(["debt", "analyze", directory.path, "--format", "json"])

        #expect(result.status == 2)
        #expect(result.stdout.isEmpty)
        #expect(result.stderr.contains(".scma.json"))
