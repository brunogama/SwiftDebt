...
struct DogfoodWorkflowTests {
    @Test("Native JSON analyzes every checked-in SCMACore source deterministically")
    func nativeJSONSelfAnalysisIsCompleteAndDeterministic() throws {
        let arguments = [
            "analyze", "Sources/SCMACore", "--format", "json", "--jobs", "1",
        ]

        let first = try runDogfoodSCMA(arguments)
        let second = try runDogfoodSCMA(arguments)
        ...
        let expectedInputs = try scmaCoreSourceNames()
    ...
    func rankedDebtSelfAnalysisUsesFrozenReferenceTime() throws {
        ...
        #expect(report.generator == "SwiftSCMA")
    ...
    private func scmaCoreSourceNames() throws -> [String] {
        let directory = dogfoodRepositoryRoot.appendingPathComponent("Sources/SCMACore")
