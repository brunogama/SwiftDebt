...
func runSCMA(_ arguments: [String], currentDirectory: URL? = nil) throws -> CLIRunResult {
    let process = Process()
...
private func scmaExecutableURL() throws -> URL {
    let direct = repositoryRoot.appendingPathComponent(".build/debug/scma")
    ...
    guard let match = matches.first else {
        throw CLIWorkflowTestError(message: "Could not locate built scma executable")
    }
    return match
}
