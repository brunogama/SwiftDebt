...
struct DebtReportRendererTests {
    ...
    @Test func nativeDebtOutputsMatchGoldenFixturesAndExposeSchemaVersion() throws {
        ...
        #expect(projection.projectionNote.contains("not the native SwiftDebt debt model"))

        let expectedJSON = try read("Tests/SwiftDebtKitTests/Fixtures/DebtReport/canonical-debt-report.golden.json")
        let expectedMarkdown = try read("Tests/SwiftDebtKitTests/Fixtures/DebtReport/canonical-debt-report.golden.md")
        let expectedDOT = try read("Tests/SwiftDebtKitTests/Fixtures/DebtReport/canonical-debt-graph.golden.dot")
        let expectedPlain = try read("Tests/SwiftDebtKitTests/Fixtures/DebtReport/canonical-debt-plain.golden.txt")
        let expectedCompact = try read("Tests/SwiftDebtKitTests/Fixtures/DebtReport/canonical-debt-compact.golden.txt")
        let expectedDebtmap = try read(
            "Tests/SwiftDebtKitTests/Fixtures/DebtReport/canonical-debtmap-projection.golden.json")
