...
struct PluginWorkflowTests {
    ...
    @Test func commandAndBuildPluginsRunDebtWorkflows() throws {
        ...
        let command = try runSwiftPackage(
            ["package", "swift-debt", "debt", "analyze", "--target", "Demo", "--format", "json"],
            in: consumer
        )
        ...
        #expect(markdown.stdout.contains("# SwiftDebt debt report"))
        ...
        let debtmap = try runSwiftPackage(
            ["package", "swift-debt", "debt", "analyze", "--target", "Demo", "--format", "debtmap-json"],
            in: consumer
        )
        ...
        #expect(stampContent == "// Generated by SwiftDebt: analysis completed. No runtime declarations.\n")
        ...
        #expect((failedBuild.stdout + failedBuild.stderr).contains("SwiftDebt [DEBT]"))
    }
    ...
    @Test func commandPluginDoesNotPrefixPlainAnalyzeTwice() throws {
        ...
        let command = try runSwiftPackage(
            ["package", "swift-debt", "analyze", "--target", "Demo", "--format", "json"],
            in: consumer
        )
    ...
    private func makePluginConsumer() throws -> URL {
        ...
        let package = Package(
            ...
            dependencies: [.package(name: "SwiftDebt", path: \(String(reflecting: repositoryRoot.path)))],
            targets: [
    ...
    private func writeConfig(_ contents: String, in root: URL) throws {
        try contents.write(to: root.appendingPathComponent(".swift-debt.json"), atomically: true, encoding: .utf8)
    }
    ...
    private func stampFiles(in root: URL) throws -> [URL] {
        ...
        return enumerator.compactMap { item in
            guard let url = item as? URL, url.lastPathComponent == "SwiftDebt.analysis.swift" else { return nil }
            return url
