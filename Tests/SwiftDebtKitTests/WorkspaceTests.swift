...
struct WorkspaceTests {
    ...
    @Test func configurationGateControlsExitStatus() async throws {
        ...
        try write("{\"thresholds\":{\"CCF\":1},\"failOnViolation\":true}", name: ".swift-debt.json", root: root)
    ...
    @Test func outputAndStampAreWrittenAfterSuccessfulAnalysis() async throws {
        ...
        let stamp = root.appendingPathComponent(".build/plugin/SwiftDebt.analysis.swift")
    ...
    @Test func failedGateDoesNotWriteStamp() async throws {
        ...
        let stamp = root.appendingPathComponent(".build/SwiftDebt.analysis.swift")
        let result = try await AnalysisService().run(
