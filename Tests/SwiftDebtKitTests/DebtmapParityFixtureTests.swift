...
struct DebtmapParityFixtureTests {
    ...
    @Test func requiredRegressionGatesHavePassingReleaseEvidence() throws {
        let matrix: ParityMatrix = try loadJSON("docs/debtmap/parity-matrix.v0.23.0.json")
        let evidence: ReleaseGateEvidence = try loadJSON("docs/debtmap/release-gate-evidence.v2.json")
        let regressionGates = try #require(matrix.capabilities.first { $0.id == "swift-debt-regression-gates" })
        ...
        #expect(
            regressionGates.proofs.contains {
                $0.kind == "gate-evidence" && $0.reference == "docs/debtmap/release-gate-evidence.v2.json"
            }
        )
        #expect(
            Set(evidence.gates.map(\.id))
                == Set([
                    "swift-build-tests-and-test-suite",
        ...
        for command in requiredCommands {
            _ = try #require(evidence.gates.first { $0.command == command })
        }
        ...
        for gate in evidence.gates {
            ...
            #expect(!gate.stdoutSummary.isEmpty)
        }

        for gate in evidence.gates where gate.id.hasSuffix("performance-budget") {
            let evidencePath = try #require(gate.evidencePath)
            let evaluation: PerformanceBudgetEvaluation = try loadJSON(evidencePath)
            #expect(gate.status == "passed")
            #expect(gate.exitStatus == 0)
            #expect(!gate.observedAt.isEmpty)
            #expect(gate.stdoutSummary.contains("PASS"))
            #expect(evaluation.comparable)
            #expect(evaluation.passed)
            #expect(evaluation.reasons.isEmpty)
        }

        let fullEvidenceReports = [
            "benchmarks/debtmap-baseline/evidence/2026-09-22-swiftdebt-rename/full-evidence-reference-report.json",
            "benchmarks/debtmap-baseline/evidence/2026-09-22-swiftdebt-rename/full-evidence-candidate-report.json",
        ]
        for path in fullEvidenceReports {
            let report: PerformanceEvidenceReport = try loadJSON(path)
            let facts = report.items.flatMap(\.evidence)
            let coverageFacts = facts.filter { $0.kind == "coverage.lcov" }
            let historyFacts = facts.filter { $0.kind.hasPrefix("git-history.") }

            #expect(coverageFacts.count == 9, "Unexpected coverage evidence count in \(path)")
            #expect(historyFacts.count == 36, "Unexpected history evidence count in \(path)")
            #expect(
                coverageFacts.allSatisfy { $0.availability.state == "measuredCoverage" },
                "Unavailable coverage evidence in \(path)"
            )
            #expect(
                historyFacts.allSatisfy { $0.availability.state == "available" },
                "Unavailable history evidence in \(path)"
            )
        }
    }
    ...
    @Test func goldenFixtureOutputsRemainDeterministic() throws {
        let fixture: GoldenFixture = try loadJSON(
            "Tests/SwiftDebtKitTests/Fixtures/DebtmapParity/canonical-workflow.fixture.json")
        ...
        let expectedJSON = try read("Tests/SwiftDebtKitTests/Fixtures/DebtmapParity/canonical-report.golden.json")
        let expectedMarkdown = try read("Tests/SwiftDebtKitTests/Fixtures/DebtmapParity/canonical-report.golden.md")
        let expectedDOT = try read("Tests/SwiftDebtKitTests/Fixtures/DebtmapParity/canonical-graph.golden.dot")
        let expectedCLI = try read("Tests/SwiftDebtKitTests/Fixtures/DebtmapParity/canonical-cli-exits.golden.json")
    ...
    @Test func benchmarkMethodologyReferencesFrozenInputs() throws {
        let methodology: BenchmarkMethodology = try loadJSON("benchmarks/debtmap-baseline/methodology.v2.json")
        ...
        #expect(methodology.platformVariance.noiseControls.contains("sixteen measured runs"))
        #expect(methodology.platformVariance.noiseControls.contains("paired interleaved reference and candidate runs"))
        #expect(methodology.platformVariance.requiredMetadata.contains("swift-version"))
        #expect(methodology.warmupRuns == 1)
        #expect(methodology.measuredRuns == 16)
        #expect(methodology.inputs.map(\.path).contains("benchmarks/debtmap-baseline/fixtures/example.lcov"))
        ...
        #expect(methodology.performanceGate.comparisonUnit == "Equivalent SwiftDebt workload only")
        #expect(methodology.performanceGate.referenceRequirements.count == 4)
        #expect(methodology.performanceGate.referenceRequirements.contains("same report schema generation"))
        ...
        #expect(methodology.regressionBudgets[1].maximumWallClockRegressionSeconds == nil)
        ...
        #expect(workflow.contains("swift-debt performance-gate"))
        #expect(workflow.components(separatedBy: "--runs 16").count - 1 == 2)
        #expect(workflow.components(separatedBy: "--paired-output").count - 1 == 2)
        #expect(workflow.components(separatedBy: "--paired-executable").count - 1 == 2)
        #expect(workflow.contains(methodology.performanceReferenceCommit))
        #expect(!workflow.contains(methodology.frozenInputProvenanceCommit))
        ...
        #expect(workflow.contains("GIT_AUTHOR_DATE=2026-09-12T12:00:00Z"))
