...
struct DebtImprovementWorkflowTests {
    ...
    @Test func goldenBeforeAfterComparisonIsDeterministicAndSchemaAware() throws {
        ...
        let expectedComparison = try read("Tests/SwiftDebtKitTests/Fixtures/DebtImprovement/comparison.golden.json")
    ...
    @Test func validationRendersMachineReadableResultAndAutomationExitStatuses() async throws {
        let beforePath = fixturePath("Tests/SwiftDebtKitTests/Fixtures/DebtImprovement/before-report.fixture.json")
    ...
    @Test func validationExecutableReturnsNonzeroStatusWhenImprovementGateFails() throws {
        let beforePath = fixturePath("Tests/SwiftDebtKitTests/Fixtures/DebtImprovement/before-report.fixture.json")
        let afterPath = fixturePath("Tests/SwiftDebtKitTests/Fixtures/DebtImprovement/after-report.fixture.json")
        let result = try runSwiftDebtExecutable(arguments: [
    ...
    private func runSwiftDebtExecutable(arguments: [String]) throws -> CommandResult {
        let executable = try swiftDebtExecutablePath()
        let process = Process()
    ...
    private func swiftDebtExecutablePath() throws -> URL {
        let executable = root.appendingPathComponent(".build/debug/swift-debt")
    ...
    private struct MissingExecutable: Error, CustomStringConvertible {
        ...
        var description: String {
            "Missing swift-debt executable at \(path); run swift build --build-tests before swift test"
        }
    }
}
