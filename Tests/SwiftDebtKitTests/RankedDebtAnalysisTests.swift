...
struct RankedDebtAnalysisTests {
    @Test func endToEndServiceProducesStableRankedDebtWithProviderEvidence() async throws {
        ...
        let request = AnalysisRequest(
            ...
            typeScope: .nominals,
            jobs: 4,
            debtAnalysisOptions: DebtAnalysisOptions(
                aggregationStrategy: .file, top: 5, problematicItemScoreThreshold: 0),
        ...
        #expect(
            analysis.items.contains { item in
        ...
        #expect(
            analysis.items.contains { item in
                item.item.evidence.contains {
                    $0.kind.hasPrefix("swift.side-effect.") || $0.kind.hasPrefix("swift.api-risk.")
                }
            })
    }
    ...
    @Test func providerUnavailabilityIsRankedAsUnavailableEvidenceNotZero() async throws {
        ...
        #expect(
            unavailable.contains {
                $0.kind.hasPrefix("git-history.") && ($0.reason ?? "").contains("not a git repository")
            })
    ...
    @Test func aggregateOnlyNoAggregationAndFiltersAreDeterministic() throws {
        let items = [
            debtItem(
                id: "callable:high", level: .callable, file: "Sources/A.swift", kind: "swift.cognitive-complexity",
                score: 90),
            debtItem(
                id: "type:low", level: .type, file: "Sources/A.swift", kind: "git-history.change-frequency", score: 20),
            debtItem(id: "file:filtered", level: .file, file: "Sources/B.swift", kind: "coverage.lcov", score: 10),
        ]

        let noAggregation = DebtAnalysisBuilder(
        ...
        let aggregateOnly = DebtAnalysisBuilder(
            options: DebtAnalysisOptions(
                aggregationStrategy: .aggregateOnly,
                problematicItemScoreThreshold: 80
            )
        ).analyze(items: items)
        let tail = DebtAnalysisBuilder(
            options: DebtAnalysisOptions(
