...
# Measurement specification - engine 0.10.0
...
The `performance-gate` command fails its wall-clock budget only when the median regression exceeds both `--max-wall-clock-regression` and the optional absolute tolerance `--max-wall-clock-regression-seconds`. It applies the same rule to peak resident memory: failure requires exceeding both `--max-peak-memory-regression` and `--max-peak-memory-regression-bytes`. Absolute tolerances default to zero, preserving percentage-only behavior when omitted. The evaluation reports the observed peak-memory byte delta, and a failure reason records both configured ceilings and the observed delta.
