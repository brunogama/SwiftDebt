...
public struct AnalysisService: Sendable {
    ...
    public func run(_ request: AnalysisRequest) async throws -> AnalysisRunResult {
        ...
        let status: Int32 =
            !report.complete ? 2 : ((failOnViolation && report.hasViolations) || debtValidationFailed ? 1 : 0)
    ...
    private func debtValidationDiagnostics(
        ...
        let failures = analysis.items.compactMap { item -> String? in
            ...
            return
                "\(absolute):\(line):\(column): warning: SCMA [DEBT] \(item.item.entity.displayName): debt score \(score) exceeds \(limit)"
        }.sorted()
