...
package struct ReportRenderer {
    ...
    private func diagnostics(_ report: AnalysisReport, root: String) -> String {
        ...
        var lines = report.diagnostics.map {
            "\(prefix($0.location)): \($0.severity.rawValue): SwiftDebt \(singleLine($0.message))"
        }
        lines += report.findings.map {
            "\(prefix($0.location)): warning: SwiftDebt [\($0.metric.rawValue)] \(singleLine($0.entity)): \($0.value) exceeds \($0.threshold)"
        }
    ...
    private func html(_ report: AnalysisReport) -> String {
        ...
        return """
            ...
            <title>SwiftDebt analysis</title><style>
            ...
            </style></head><body><header><p class="meta">STATIC ANALYSIS / SCMA PAPER IMPLEMENTATION</p><h1>SwiftDebt</h1>
            ...
            <footer>SwiftDebt \(report.engineVersion) · Report schema \(report.schemaVersion) · Self-contained report; no scripts, analytics, or network assets.</footer>
            </body></html>
            """ + "\n"
    }
