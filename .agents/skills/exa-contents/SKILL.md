...
| --- | --- | --- | --- | --- |
| `urls` | string[] | One of `urls` or `ids` | - | URLs to extract content from. Use this for known URLs. |
| `ids` | string[] | One of `urls` or `ids` | - | Document IDs returned by `/search`. Use this to re-read specific search results without reconstructing URLs. |
