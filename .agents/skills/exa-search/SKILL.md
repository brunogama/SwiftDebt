...
| `category` | string | No | - | Specialized result type: `company`, `people`, `publication`, `news`, `personal site`, or `financial report`. |
| `includeDomains` | string[] | No | - | Only return results from these domains, paths, or wildcard patterns. Max 1200. |
| `excludeDomains` | string[] | No | - | Exclude these domains, paths, or wildcard patterns. Max 1200. Not supported with the `company` or `people` categories. |
...
Treat streaming as SSE rather than JSON. Frames are not all chat completion chunks: branch on the frame type before reading. Read partial text only from chunk frames (`choices[0].delta.content`), skip reset/grounding/other non-chunk frames, and handle completion or error frames defensively.
...
- Use documented categories only: `company`, `people`, `publication`, `news`, `personal site`, and `financial report`.
