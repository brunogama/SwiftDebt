...
The `regex` rule filters candidate nodes by testing a Rust regular expression against the node's text. Matching is **unanchored substring** matching: a pattern like `foo` matches any node whose text contains `foo`, not only nodes whose text is exactly `foo`. Use `^...$` anchors when whole-node matching is intended. `regex` only filters nodes already matched by other rules, so it still needs a positive matcher like `kind` or `pattern` in the same rule to select candidates.
